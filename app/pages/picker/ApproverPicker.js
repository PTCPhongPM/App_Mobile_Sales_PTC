...
import { useGetApproverQuery } from "../../store/api/account";
import PickerItem from "../../components/Picker/ItemPicker";

const { TextField } = Incubator;

// todo remove because not choose approver
const ApproverPicker = ({ navigation, route: { params } }) => {
  const handleBack = useCallback(() => navigation.goBack(), [navigation]);

  const [filter, setFilter] = useState("");

  const { data, isFetching, refetch } = useGetApproverQuery();

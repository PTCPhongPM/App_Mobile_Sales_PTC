...
import { useGetSalePolicyListAllQuery } from "../../store/api/salePolicy";

const PolicyPicker = ({ navigation, route: { params } }) => {
  const { selected, onSelect, productCode } = params;
  const handleBack = useCallback(() => navigation.goBack(), [navigation]);

  const { data, refetch, isFetching } = useGetSalePolicyListAllQuery();
  ...
  const matched = useMemo(() => {
    return (
      data?.filter((e) => {
        if (!e.productCode || !productCode) return true;

        const arr = e.productCode.split(",").map((e) => e.trim());

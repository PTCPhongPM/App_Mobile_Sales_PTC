...
const GeneralTestDriveEditor = ({ navigation, route: { params } }) => {
  ...
  const defaultValues = useMemo(() => {
    ...
    return {
      ...testDrive,
      startingTime: testDrive.startingTime.toDate(),
      endingTime: testDrive.endingTime.toDate(),
      files: savedFiles,
    };
  ...
  const handleSave = useCallback(
    async (data) => {
      ...
      if (isEdit) {
        ...
        Alert.alert(
          "Yêu cầu phê duyệt",
          "Bạn sẽ gửi yêu cầu phê duyệt đến quản lý đã được chọn",
          [
            {
              text: "Lưu bản nháp",
              onPress: async () => {
  ...
  const handleAddPhoto = useCallback(() => {
    navigation.navigate("PhotoEditor", {
      ...
      files,
      onChange: (value) => {
        setValue("files", value, { shouldValidate: true });
      },
    });
  }, [customer, files, navigation, setValue, testDrive]);

  // const handleChooseApprover = useCallback(
  //   () =>
  ...
  return (
    <BasePage loading={loading}>
      <View bg-white padding-16 style={[gStyles.borderV, gStyles.shadow]}>
        <TouchableOpacity row center paddingV-8 onPress={handleChooseProduct}>
...
GeneralTestDriveEditor.propTypes = {
...
export default GeneralTestDriveEditor;
